...
import com.assignment.spring.models.*;
import com.assignment.spring.security.CustomAuthenticationProvider;
import com.assignment.spring.security.jwt.JwtUtils;
import com.assignment.spring.payload.request.LoginRequest;
import com.assignment.spring.payload.request.SignupRequest;
import com.assignment.spring.payload.request.UpdateRequest;
import com.assignment.spring.payload.response.MessageResponse;
import com.assignment.spring.payload.response.UserListResponse;
import com.assignment.spring.payload.response.UserResponse;
import com.assignment.spring.repository.ConfirmationTokenRepository;
import com.assignment.spring.repository.FileDBRepository;
...
//    emailSenderService.sendEmail(mailMessage);
  ...
  public User saveUser(long id, UpdateRequest updateRequest) {
    ...
    if (user.isPresent()) {
      User addUser = user.get();
      if (updateRequest.getEmail() != null)
        addUser.setEmail(updateRequest.getEmail());
      if (updateRequest.getPassword() != null)
        addUser.setPassword(encoder.encode(updateRequest.getPassword()));
      addUser.setUsername(updateRequest.getUsername());
      addUser.setAddress(updateRequest.getAddress());
      addUser.setAge(updateRequest.getAge());
      addUser.setCity(updateRequest.getCity());
      addUser.setCountry(updateRequest.getCountry());
      addUser.setDescription(updateRequest.getDescription());
      addUser.setJob(updateRequest.getJob());
      addUser.setName(updateRequest.getName());
      addUser.setSurname(updateRequest.getSurname());
